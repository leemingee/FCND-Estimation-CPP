...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // updated and workable version
  Quaternion<float> qt_new = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  qt_new.IntegrateBodyRate(gyro, dtIMU);

  float predictedPitch = qt_new.Pitch();
  float predictedRoll = qt_new.Roll();
  ekfState(6) = qt_new.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(5) += (accel_body.z - CONST_GRAVITY)*dt;
